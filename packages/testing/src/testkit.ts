...
/** Bounds every wait so that a regression fails the run instead of hanging it. */
const SETTLE_TIMEOUT = 5;
const NEVER_SETTLED = "@flamework/testing: the promise never settled";

/**
 * Waits for `promise` to settle.
 *
 * Awaiting yields, so anything the promise is waiting on -- a deferred remote connection, a
 * `Promise.delay` timeout -- gets a chance to run.
 */
