...
import type { InstanceShape } from "./instanceTree";
...
export interface ComponentConfig {
	 ...
	 *
	 * A guard written here is re-run whole whenever the tree changes, and can only say that it
	 * failed; the shape the transformer writes from the instance type is watched one child at a
	 * time and says which child is wrong.
	 */
	instanceGuard?: t.check<unknown>;

	/**
	 * The instance tree, as data: the classes the instance may be and the children it must have,
	 * each with a shape of its own. Written by the transformer from the component's instance type
	 * wherever that type is only classes and children; there is no reason to set this by hand.
	 */
